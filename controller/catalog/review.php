...
class ControllerCatalogReview extends Controller {
	...
	public function edit() {
		...
		$results = $this->model_catalog_review->getDetails($filter_data, $this->request->get['survey_id']);

		foreach ($results as $result) {
			$data['reviews'][] = array(
				'review_id'  => $result['user_id'],
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['reviews'][] = array(
				...
				/*'status'     => ($result['status']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
				'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),*/
				'edit'       => $this->url->link('catalog/review/edit', 'user_token=' . $this->session->data['user_token'] . '&survey_id=' . $result['survey_id'] . $url, true)
			);
		}
